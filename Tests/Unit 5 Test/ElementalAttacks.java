public interface ElementalAttacks {
    ...
    default void attack(Monster opponent) {
        ...
        if (opponent == null) {
            throw new IllegalArgumentException("Cannot attack a nonexistent opponent!");
        }
    ...
    // The current monster attacks the opponent using the attackNumber provided
    // Inputs:
    ...
    default void performSingleElementalAttack(int attackNumber, Monster opponent) {
        ...
        if (attackNumber < 1 || attackNumber > 2) {
            throw new IllegalArgumentException("Attack number out of range - must be 1 or 2!");
        }

        Monster thisMonster = thisMonster();
        ElementType atkElement = null;
        String atkName = null;

        if (attackNumber == 1) {
            atkName = thisMonster.attackNameOne;
            atkElement = thisMonster.attackElementOne;
        } else {
            atkName = thisMonster.attackNameTwo;
            atkElement = thisMonster.attackElementTwo;
        }

        int atk = thisMonster.getAttack();
        int oppDef = opponent.getDefense();
        double STAB = 1.0;
        if (atkElement.equals(thisMonster.getElement())) {
            STAB = 1.5;
        }
        double effectivenessMultiplier = getElementalMultiplier(atkElement, opponent.getElement());
        ...
        // sout section
        System.out.println(thisMonster.getName() + " used " + atkName + "!");
    ...
    default void performDoubleDownAttack(Monster opponent) {
        ...
        if (thisMonster.hasUsedDoubleDown()) {
            System.out
                    .println(thisMonster.getName() + " has already used their double down attack!");
            return;
        } else {
            System.out.println(thisMonster.getName() + " uses their double down attack!");
